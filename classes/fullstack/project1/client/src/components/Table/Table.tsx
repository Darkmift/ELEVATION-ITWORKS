...
const THeader: React.FC<ReactNodeProps> = ({children, classNames}) => {
    ...
    return (
        <thead className={classes}>{children}</thead>
    )
}
...
const TD: React.FC<ReactNodeProps> = ({children, classNames}) => {
    let classes = classNames || "";
    classes += " text-[12px] text-[#17181C]";
    const svgSuccess = (
        <svg
            viewBox="0 0 18 18"
            fill="none"
    ...
    if (children == "Success") {
        classes += " border-l-2 border-s-[#00D2A2]";
        imgNode = svgSuccess;
        
    }
    if (children == "Failed") {
        classes += " border-l-2 border-s-[#E12828]";
        imgNode = svgFailed;
    }
    if (children == "Cancelled") {
        classes += " border-l-2 border-s-[#374A5080]";
        imgNode = svgCancelled;
    }
    return (
        <td className={classes}>{imgNode}{children}</td>
    )
}
...
const TBody: React.FC<ReactNodeProps> = ({children, classNames}) => {
    const classes = classNames || "";
    return (
        <tbody className={classes}>{children}</tbody>
    )
}
